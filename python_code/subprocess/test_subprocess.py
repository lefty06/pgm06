...
def example4():
    ...
    #Method 2
    str_subprocess_out=subprocess.Popen("echo subprocess execute", stdout=subprocess.PIPE, shell=True).stdout.read()
    print 'str_subprocess_out:\n', str_subprocess_out

    print '-------------------------'

    #Method 3
    str_subprocess_out2=subprocess.Popen("ls subprocess execute", stdout=subprocess.PIPE, shell=True)
...
def example5():
    ...
    proc = subprocess.Popen(['ls', '"Hello world!"'],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
...
def example6():
    ...
    proc = subprocess.Popen(['ls', '"Hello world!"'],stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
    (stdoutdata, stderrdata) = proc.communicate()
    print 'stdoutdata:\n', stdoutdata
    print 'stderrdata:\n', stderrdata
    print 'return code:\n', proc.returncode
...
def example7():
    ...
    proc = subprocess.Popen(['/bin/bash','/home/pat/Documents/python_scripts/subprocess/subprocess_example7.bash'],  stdin=subprocess.PIPE)
    print proc.communicate(' Hello?')
    print 'return code: ', proc.returncode

def example8():
    #this simulated s user input (check bash script) and then captures stdout and stderror
    proc = subprocess.Popen(['/bin/bash','/home/pat/Documents/python_scripts/subprocess/subprocess_example7.bash'],  stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
...
def example9():
    ...
    p2 = subprocess.Popen(['grep','-A1','system'], stdin=p1.stdout, stdout=subprocess.PIPE,stderr=subprocess.PIPE)
